...
isPrime' n = do
 lastCalcNum <- gets lastCalculatedNum
 if (n > lastCalcNum) 
 then do
  root        <- return $ floor (sqrt (fromIntegral n))
  when (lastCalcNum < root) ((getDivisors' root) >> return ())
  findFirstPrimeDivisor n >>= return . isNothing
 else do
  divs <- getDivisors' n
  case divs of
   (Pr)      -> True
   (DIVS _)  -> False
   (_)       -> error ("uncalculated n: " ++ show n)   
...
lazyPrimeDivisionUnion pr d2 = return []
