...
newDivisonState :: Integer -> IO (DivisionStateData a) 
...
newOnlyDivisorState :: Integer -> IO (DivisionStateData ())
newOnlyDivisorState = newDivisonState

commonWrite selector n dat = do
 arr <- gets array
 liftIO $ writeArray (selector arr) n dat

writeDivisors :: (MonadState (DivisionStateData a) m, MonadIO m) => Integer -> DivisionData -> m ()
writeDivisors = commonWrite divisorArray

writeUserData :: (MonadState (DivisionStateData a) m, MonadIO m) => Integer -> a -> m ()
...
readDivisors :: (MonadState (DivisionStateData a) m, MonadIO m) => Integer -> m DivisionData
readDivisors = commonRead divisorArray

readUserData :: (MonadState (DivisionStateData a) m, MonadIO m) => Integer -> m (Maybe a)
readUserData n = commonRead userArray n

isPrime :: (MonadState (DivisionStateData a) m, MonadIO m) => Integer -> m Bool
isPrime n = do
 divData <- readDivisors n
 case divData of
  (Pr _) -> return True
  (_)    -> return False  
...
copyArrayToArray oldArr newArr oldCap = 
 sequence_ [ readArray oldArr i >>= writeArray newArr i | i <- range (1, oldCap)]


increaseArraySizes :: Integer -> Integer -> Array a -> IO (Array a)
increaseArraySizes oldCap newCap (Arr divArr usrArr) = do
 newDivArr  <- newArray (1, newCap) Uncalculated
 newUserArr <- newArray (1, newCap) Nothing
 copyArrayToArray divArr newDivArr oldCap
 copyArrayToArray usrArr newUserArr oldCap
 return (Arr newDivArr newUserArr)


checkAndIncreaseArraySizes :: Integer -> Division a ()
checkAndIncreaseArraySizes n = do
 (DSD lastPr arr) <- get
 upperBound       <- (liftIO . getBounds . divisorArray) arr >>= (return . snd)
 when 
  (upperBound < n) 
  (get >>= liftIO . (increaseArraySizes upperBound n) . array >>= put . (DSD lastPr))


calculateDivisors :: Integer -> Division a () -- [Divisor]
calculateDivisors n = do
 checkAndIncreaseArraySizes n
 (DSD lastPr arr) <- get
...
getNewSup supToChange targetSup
 | supToChange < targetSup = getNewSup (2 * supToChange) targetSup
 | otherwise               = supToChange   

 
lazyPull :: UnionF () -> DivisionStateData () -> Integer -> IO (DivisionStateData ())
lazyPull unionF dsd sup = do
 oldSup <- (getBounds . divisorArray . array) dsd >>= return . snd
 if (oldSup >= sup) 
 then return dsd
 else do 
  newSup    <- return $ getNewSup oldSup sup
  newArrays <- increaseArraySizes oldSup newSup (array dsd)
  execStateT 
   (runReaderT (mapM_ calculateDivisors [(oldSup + 1) .. sup]) unionF) 
   (DSD (lastPrime dsd) newArrays)


getDivisors unionF cap = do
 dsd <- newDivisonState cap
 execStateT (runReaderT (mapM_ calculateDivisors [1 .. cap]) unionF) dsd
