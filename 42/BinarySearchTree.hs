...
instance BinarySearchTree ScapeGoatBinaryTree where
 ...
 findElem a = (findElemInBinaryTree a) . binaryTree
...
findElemInBinaryTree :: Ord a => a -> BinaryTree a -> Maybe a
findElemInBinaryTree a NIL                  = Nothing
findElemInBinaryTree a (BT left elem right) = 
 case (compare elem a) of
 (LT) -> findElemInBinaryTree a right
 (EQ) -> Just elem
 (GT) -> findElemInBinaryTree a left
